EyeModel_overlap_N2O<- function (CoorX, CoorY, Variable, CoorX_sub, CoorY_sub, Variable_sub, Direccion, Tol, NIntervalos, 
          Lags, Npares, Modelo, Nugget, SillYNugget, Alcance, MainTitle="", xlab="", ylab = "") 
{
  ...
  MaxX <-1.1*max(Nuevo$u, variograma_sub$u, variograma_sub2$u) # MaxX <- max(variograma$u) + 0.2 * max(variograma$u)
  MaxY <-1.1*max(Nuevo$v, Nuevo$var.mark, variograma_sub$v, variograma_sub$var.mark,
                 variograma_sub2$v, variograma_sub2$var.mark)
  ...
  plot(Nuevo$u, Nuevo$v, pch = 21, col = "black", bg = "blue", 
       xlim = c(0, MaxX), ylim = c(0, MaxY), xlab = xlab, 
  ...
  plot.default(variograma_sub$u, variograma_sub$v, type = "p", pch = 19, col = "red", 
               ...
               ylab = ylab, cex.lab = 1.5, cex.axis = 1.2, cex.main=1.5,
               main = MainTitle)
  par(new=TRUE)
  ...
  legend(x = "bottomright", y = NULL, c(Modelo, "Variance", "Sill+Nugget", "Range"), 
